...
Feature: Verify Calculator canvas functionalities
  ...
  Background: Open Calculator and Switch to iframe=fullframe
    Given I open Calculator Canvas
    When I switch to iframe=fullframe

  @regression @smoke @single-operator @addition
  Scenario Outline: Verify calculator using single operator: Addition
  ...
  @regression @smoke @single-operator @subtraction
  Scenario Outline: Verify calculator using single operator: Subtraction
    When I enter value into Calculator = "<value1>"
    And I enter value into Calculator = "<operator>"
    And I enter value into Calculator = "<value2>"
    And I enter value into Calculator = "Keys.EQUALS"
    ...
    Examples:
      ...
      | 2      | Keys.SUBTRACT | 3      | -1              |
      | -2     | Keys.SUBTRACT | 3      | -5              |
      # Failed: Got issue with send -3 for this case. It understands as Subtract & 3.
      # Expect: -2 -(-3)No = 1. Actual: -2 -(-3) = -5
      | -2     | Keys.SUBTRACT | -3     | 1               |
      | 90     | Keys.SUBTRACT | 21     | 69              |
      |        | Keys.SUBTRACT | 1      | -1              |
      | 1      | Keys.SUBTRACT |        | 0               |
      | a      | Keys.SUBTRACT | 1      | -1              |
...
@regression @smoke @single-operator @division
Scenario Outline: Verify calculator using single operator: Division
    When I enter value into Calculator = "<value1>"
    And I enter value into Calculator = "<operator>"
    And I enter value into Calculator = "<value2>"
    And I enter value into Calculator = "Keys.EQUALS"
    ...
    Examples:
      ...
      | 1      | Keys.DIVIDE   | 1      | 1               |
      | -1     | Keys.DIVIDE   | 1      | -1              |
      # Fail: Got issue when send second -1. It understands as SUBSTRACT & 1
      # Expect: 1 :(-1) = 1. Actual: 1 : (-1) = 0
      | -1     | Keys.DIVIDE   | -1     | 1               |
      | 1      | Keys.DIVIDE   | 2      | 0.5             |
      | -1     | Keys.DIVIDE   | 2      | -0.5            |
      | 10     | Keys.DIVIDE   | a      | 1               |
      ...
      | 0      | Keys.DIVIDE   | 10     | 0               |
      | 100    | Keys.DIVIDE   | 20     | 5               |
      | 1000   | Keys.DIVIDE   | 20     | 50              |
